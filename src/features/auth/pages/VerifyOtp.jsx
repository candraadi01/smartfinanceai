...
export default function VerifyOtp() {
  ...
  const canSubmit = useMemo(() => email.trim() && otpCode.length === otpLength && !loading, [email, loading, otpCode, otpLength])
  ...
  async function resendOtp() {
    ...
    if (!email.trim()) {
      setError('Isi email terlebih dahulu sebelum mengirim ulang OTP.')
      return
    }

    try {
      await authRepository.resendOtp({ email: email.trim().toLowerCase(), purpose: 'register' })
      setStatus(!isApiMode ? 'OTP baru sudah dikirim. Untuk mode lokal gunakan kode 123456.' : 'OTP baru sudah dikirim ke email kamu.')
  ...
  return (
    <div className="auth-page">
      <div className="auth-card">
        ...
        {!isApiMode && <p className="helper-text">Mode lokal: gunakan OTP <b>123456</b>.</p>}
        {!initialEmail && <p className="helper-text">Buka halaman ini dari proses register, atau isi email yang kamu daftarkan.</p>}
        ...
        <form onSubmit={submit}>
          ...
          <label>
            ...
            <input
              ...
              placeholder="123456"
              value={otpCode}
              onChange={(event) => setOtpCode(event.target.value.replace(/\D/g, '').slice(0, otpLength))}
            />
          </label>
          <button className="btn full" disabled={!canSubmit}>{loading ? 'Memverifikasi...' : 'Verifikasi OTP'}</button>
        </form>
